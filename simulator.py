...
def validate_canonical_form(proc: hpc.Process) -> Tuple[bool, str]:
    ...
    if m_count == 0 and c_count == 0 and r_count == 0:
        return False, "Standard form has no active M/C/R processes"
...
def normalize_numeric_artifact(value: Any) -> Any:
    """Remove tiny floating-point artifacts introduced by numeric integration."""
...
def safe_eval_expr(expr: str, env: Dict[str, float], par: Optional[hpc.Parallel] = None) -> Tuple[Any, Set[str]]:
    ...
    for ident in ids:
        if ident in math_funcs or ident in math_consts:
            continue
        _SAFE_EVAL_DEPTH += 1
        try:
            if ident in env:
...
def apply_pass_rule(par: hpc.Parallel) -> Tuple[bool, hpc.Process, Optional[str]]:
    ...
    for i, proc in enumerate(new_processes):
        if isinstance(proc, hpc.PrefixProcess) and isinstance(proc.prefix, hpc.Guard):
            condition = proc.prefix.condition
            cond_val, missing = safe_eval_expr(condition, global_env, par)
            if missing:
                continue
            if cond_val is True:
                new_processes[i] = proc.continuation
                applied = True
    ...
    for cont_idx, cont_comp, ode, repl in cont_items:
        ...
        for in_info in discrete_inputs:
            ...
            if ((in_ch in ready_set) or (in_ch in alias_map)) and len(in_prefix.var_list) == 1:
                ...
                if e_val is None:
                    continue
                e_val = normalize_numeric_artifact(e_val)
    ...
    if isinstance(proc, hpc.Parallel):
        for sub in getattr(proc, "processes", []):
            if isinstance(sub, hpc.Inaction):
                continue
            if is_auxiliary_runtime_process(sub):
                continue
...
def is_auxiliary_runtime_process(proc: hpc.Process) -> bool:
    """Runtime helpers used by the simulator should not block normal termination."""
    if proc is None or isinstance(proc, hpc.Inaction):
        return True

    if isinstance(proc, hpc.PrefixProcess):
        pref = getattr(proc, "prefix", None)
        cont = getattr(proc, "continuation", None)

        if isinstance(pref, hpc.Continuous):
            ode = pref.ode
            is_clock = (
                list(getattr(ode, "v", []) or []) == ["clock"]
                and list(getattr(ode, "e", []) or []) == ["1"]
                and isinstance(cont, hpc.Inaction)
            )
            if is_clock:
                return True
    ...
    if isinstance(proc, hpc.NamedProcess):
        ...
        if name == "GlobalClock":
            return True
        return is_auxiliary_runtime_process(body)
